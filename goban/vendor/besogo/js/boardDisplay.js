besogo.makeBoardDisplay = function(container, editor) {
    ...
    function initializeBoard(coord) {
        ...
        tetherGroup = besogo.svgEl("g");
        svg.appendChild(tetherGroup);
        ...
        setupTether(); // Pointer-driven tether for touch (optional mouse drag)
    ...
    function setupTether() {
        ...
        svg.addEventListener('pointerdown', onTetherPointerDown);
        svg.addEventListener('pointermove', onTetherPointerMove);
        svg.addEventListener('pointerup', onTetherPointerUp);
        svg.addEventListener('pointercancel', onTetherPointerCancel);
    }
    ...
    function nearestIntersection(clientX, clientY, prevI, prevJ) {
        var pt = clientToSvg(clientX, clientY),
            current = editor.getCurrent();
        return besogo.tetherMath.nearestIntersectionFromSvg(pt.x, pt.y, sizeX, sizeY, {
            cellSize: CELL_SIZE,
            boardMargin: BOARD_MARGIN,
            prevI: prevI,
            prevJ: prevJ,
            retargetHysteresis: RETARGET_HYSTERESIS,
            hasStone: function (i, j) {
                return !!current.getStone(i, j);
            }
        });
    }
    ...
    function onTetherPointerMove(event) {
        if (tetherActive && event.pointerId === tetherPointerId) {
            updateTether(event);
            event.preventDefault();
            return;
    ...
    function onTetherPointerUp(event) {
        if (tetherActive && event.pointerId === tetherPointerId) {
            endTether(event);
            event.preventDefault();
            return;
