besogo.makeControlPanel = function(container, editor) {
    ...
    var leftElements = [], // SVG elements for previous node buttons
        deleteElements = [], // SVG elements for delete move button
    ...
    function update(msg) {
        ...
        if (msg.navChange || msg.treeChange) { // Update the navigation buttons
            ...
            if (current.parent && current.children.length === 0) { // Leaf, not root
                arraySetColor(deleteElements, 'black');
            } else {
                arraySetColor(deleteElements, besogo.GREY);
            }
        }
    ...
    function drawNavButtons() {
        ...
        deleteElements.push(makeNavButton('Delete move (leaf only; use Cut for branches)',
